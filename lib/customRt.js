...
async function fetchSepta(agency) {
  ...
  const vehicles = data
    ...
    .map(t => ({
      ...
      heading: isNaN(parseFloat(t.heading)) ? null : parseFloat(t.heading),
...
async function fetchMetroTransit(agency) {
  ...
  const vehicles = data
    ...
    .map(v => ({
      ...
      lat: v.latitude,
      lng: v.longitude,
      heading: v.bearing != null && !isNaN(v.bearing) ? v.bearing : null,
  ...
  const vehicles = list
    ...
    .map(v => {
      ...
      return {
        ...
        lat: v.location.lat,
        lng: v.location.lon,
        heading: v.tripStatus?.orientation != null && !isNaN(v.tripStatus.orientation) ? v.tripStatus.orientation : null,
        routeId,
        agencyId: agency.id,
        agencyName: agency.name,
        realtime: true,
...
async function fetchAmtrak(agency, routeSlugMap) {
  ...
  try { data = await fetchJson('https://api.amtraker.com/v3/trains'); }
    ...
    try { data = await fetchJson('https://api.amtraker.com/v2/trains'); }
    catch (e2) { console.warn('[amtrak] RT failed:', e2.message); return null; }
  }
