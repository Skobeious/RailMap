...
// Golden-ratio hue spacing — produces maximally distinct colours on dark backgrounds
function buildRouteColorMap(routeIds) {
  const GOLDEN = 0.6180339887;
  const map = {};
  [...routeIds].sort().forEach((id, i) => {
    const hue = ((i * GOLDEN) % 1) * 360;
    map[id] = `hsl(${hue.toFixed(0)},70%,62%)`;
  });
  return map;
}
...
async function loadAgency(agency) {
  ...
  if (!agency.gtfsUrl) {
    console.warn(`[${agency.id}] no gtfsUrl — skipped`);
  ...
  // Some agencies (SEPTA) wrap rail + bus as separate zips inside the outer zip
  const railEntry = zip.getEntries().find(e => /rail/i.test(e.entryName) && e.entryName.endsWith('.zip'));
  const innerZip = railEntry ? new AdmZip(zip.readFile(railEntry)) : null;

  const getText = (name) => {
    const src = innerZip || zip;
    const e = src.getEntry(name);
    return e ? src.readAsText(e) : '';
  };

  const routeTypes = agency.routeTypes;

  // Parse routes — keep only matching types (and allowedRouteIds if set)
  const allRoutes = parseCSV(getText('routes.txt'));
  const routeMap = {};
  allRoutes.forEach(r => {
    if (!routeTypes.includes(parseInt(r.route_type || '3'))) return;
    if (agency.allowedRouteIds && !agency.allowedRouteIds.has(r.route_id)) return;
  ...
  const perRouteColors = agency.perRouteColors
    ? buildRouteColorMap(new Set(Object.values(shapeToRoute)))
    : null;
  ...
  Object.entries(shapePoints).forEach(([shapeId, pts]) => {
    ...
    const color = perRouteColors
      ? perRouteColors[routeId]
      : agency.overrideColor || (route.route_color ? '#' + route.route_color : '#888888');
    features.push({
      type: 'Feature',
      properties: {
        shapeId,
  ...
  // Don't expose stops if no rail shapes loaded — avoids orphaned bus stop markers
  const data = { agencyId: agency.id, agencyName: agency.name, features, stops: features.length ? stops : [] };
